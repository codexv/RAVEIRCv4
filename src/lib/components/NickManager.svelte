...
<style>
  ...
  /* WebKitGTK (Linux) renders native <select> with the GTK theme (white box),
     ignoring background/color. Force our own styling + a custom dropdown arrow. */
  select {
    appearance: none;
  ...
  /* Style the open dropdown list items too (otherwise white on Linux). */
  select option {
    background: var(--bg);
    color: var(--fg);
  }
